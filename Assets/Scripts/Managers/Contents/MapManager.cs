...
public class MapManager
{
    ...
    public List<Vector3Int> FindPath(Vector3Int startCellPos, Vector3Int destCellPos, bool ignoreDestCollision = false)
    {
        ...
        bool[,] closed = new bool[SizeY, SizeX];//ClostList
        ...
        pq.Push(new PQNode() { F = 10 * (Math.Abs(dest.Y - pos.Y) + Math.Abs(dest.X - dest.Y)), G = 0, Y = pos.Y, X = pos.X });
        ...
        while(pq.Count > 0)
        {
            //제일 좋은 후보를 찾는다
            PQNode node = pq.Pop();
            //동일한 좌표를 여러 경로로 찾아서, 더 빠른 경로로 인해 이미 방문(closed)된 경우 스킵한다.
